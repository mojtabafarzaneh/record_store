...
async def insert_records(records : CreateRecords,db: AsyncSession = Depends(get_async_db)):
...
async def read_records(db: AsyncSession = Depends(get_async_db)):
...
async def read_record(records_id: uuid.UUID, db: AsyncSession = Depends(get_async_db)):

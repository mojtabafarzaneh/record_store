...
async def insert_card(card : CreateCard,user_id: uuid.UUID, record_id: uuid.UUID,db: AsyncSession = Depends(get_async_db)):
...
async def read_card(card_id : uuid.UUID, db:AsyncSession = Depends(get_async_db)):
...
async def read_cards(db: AsyncSession = Depends(get_async_db)):
